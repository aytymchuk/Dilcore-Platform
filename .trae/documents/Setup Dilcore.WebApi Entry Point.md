...
### Plan Steps:
1.  **Create Project**: Run `dotnet new web -n Dilcore.WebApi` in `src/`.
2.  **Configure Inheritance**: Edit `Dilcore.WebApi.csproj` to remove `<TargetFramework>` so it inherits `net10.0` from `Directory.Build.props`.
3.  **Setup Production Dockerfile**: Create a multi-stage `Dockerfile` in `src/Dilcore.WebApi/`:
...
6.  **Update Solution**: Add `Dilcore.WebApi` to `Dilcore.Platform.sln`.