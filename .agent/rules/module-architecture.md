...
* **Role**: Contains DTOs intended for boundary translation.
* **Responsibilities**:
...
* **Responsibilities**:
  ...
  * Processes functionality via appropriate actors or store repositories (defined as abstractions).
  * **Note**: `Store`, `Actors`, and `Infrastructure` depend on and implement abstractions defined in `Core`. `Core` does NOT depend on them.
* **Allowed Dependencies**:
  * `Module.Domain`
  * `Module.Actors.Abstractions`
