...
{
    /// <summary>
    /// Executes a void API call and wraps the outcome in a Result with comprehensive error handling.
    /// </summary>
    /// <param name="apiCall">The API call to execute.</param>
    /// <returns>Result indicating success or error information.</returns>
    internal static async Task<Result> InvokeAsync(Func<Task> apiCall)
    {
        try
        {
            await apiCall();
